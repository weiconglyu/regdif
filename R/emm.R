...
#' @examples
#' \dontrun{
#' result <- with(regDIF_simdata, regDIF_EM(Y, D, X))}
regDIF_EM <- function(Y, D, X, Method = 'EMM', Unif = F, Lambda0 = seq(0.2, 0.7, by = 0.1), iter = 1000, eps = 1e-3, c = 0.7, eta = 1) {
...
#' @examples
#' \dontrun{
#' result <- with(regDIF_simdata, regDIF_LRT(Y, D, X))}
regDIF_LRT <- function(Y, D, X, Unif = F){
  X <- as.factor(X)
  resp <- as.data.frame(Y)
  if (min(resp) == 0) {
    resp2 <- as.matrix(resp)
